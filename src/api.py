"""FastAPI application for ai-ethics-simulator."""
...
app = FastAPI(title="ai-ethics-simulator", version="0.1.0", description="ai-ethics-simulator API")
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "ai-ethics-simulator", "version": "0.1.0"}
...
async def process(req: ProcessRequest):
    ...
    try:
        ...
        resp = ProcessResponse(success=True, result=result, processing_time_ms=round(elapsed, 2),
                              metadata={"service": "ai-ethics-simulator", "input_length": len(req.input)})
...
@app.get("/status")
def status():
    return {"service": "ai-ethics-simulator", "version": "0.1.0", "ready": True,
